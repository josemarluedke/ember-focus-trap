...
export default class ThemeSwitcher extends Component {
  ...
  constructor(owner: never, args: Record<string, unknown>) {
    super(owner, args);
    if (typeof document === 'undefined') {
      return;
    }
    const root = document.documentElement;
    this.prefersDark = root.classList.contains(DARK_MODE_CLASS);
  ...
  <template>
    <button
      type="button"
      class="transition text-gray-300 hover:text-white outline-none focus-visible:ring rounded p-1"
      aria-label={{if this.prefersDark "Switch to light mode" "Switch to dark mode"}}
      ...attributes
